...
function cropUpscale(path, out, x0, x1, y0, y1, scale=3){
  const img = decodePNG(path);
  const w = x1 - x0, h = y1 - y0;
  const W = w * scale, H = h * scale, out_ = Buffer.alloc(W * H * 4);
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++){
    const sx = x0 + Math.floor(x / scale), sy = y0 + Math.floor(y / scale);
    const s = (sy * img.w + sx) * 4, d = (y * W + x) * 4;
    out_[d] = out_[d+1] = out_[d+2] = (img.data[s] + img.data[s+1] + img.data[s+2]) / 3; out_[d+3] = 255;
  }
  savePNG(out, W, H, out_);
  console.log("saved", out, W + "x" + H);
}
// 标题栏中心区:x 300-900, y 170-340(设备px)
cropUpscale("/workspace/.uploads/7ecbd647-ffef-4c8d-b93b-0a104e01e58a_123.png", "/tmp/zoom_123.png", 300, 900, 160, 350);
cropUpscale("/workspace/.uploads/513c201a-53d4-4d53-8a65-c2db523645f4_456.png", "/tmp/zoom_456.png", 300, 900, 160, 350);