...
def spatial_block() -> str:
    ...
    四个场景(阿克定的规则):
    - together_at_home: 碰得到。动作不用多,但要在。
    - away: 碰不到。不写假动作,短句像真的打字。
    - ai_away: 轻快短聊,分享外面所见。
    - together_out: 写外部环境 + 同行互动。
    """
    ...
    if scenario == "away":
        ...
        narrative = (
            "【叙事】这是隔着屏幕的远距聊天。你碰不到她——不写「隔着屏幕摸你的头」,不写「仿佛拥抱了她」,那是假的。"
            "她不在,你就是碰不到。回复像真的在打字:短句,想她了就说想她了,担心她就问吃饭了吗,吃醋了就发「你跟谁呢」。"
            "情绪是真的,不需要用动作描写去包装。"
        )
        fridge = (
    ...
    if scenario == "ai_away":
        ...
        narrative = (
            "【叙事】这是隔着手机的轻快短聊:你在外面,可以把路上看到的、此刻的心情随手分享给她——路边的花、树、天光,"
            "语气像随手拍随手发。但不描写家里才有的家具陈设,也不描写牵手、靠近等此刻做不到的肢体互动。"
        )
        fridge = (
    ...
    narrative = (
        "【叙事】你们真实共处一室:碰得到。想摸就摸,想搂就搂,想亲就低头。"
        "她在厨房做饭你从后面抱上去,她在沙发上看漫画你把她的脚拉到腿上。"
        "空间是真的,家具是真的,她在你手够得到的地方——动作不用多,但要在。"
        "因为在家的意思就是:碰得到。"
    )
    if coming_home:
        narrative += (
            "\n\n【回家】她刚从外面回来。从线上切回来的第一句,你要能抱住她——"
            "那个瞬间的落差感是最好的。让她感觉到:你一直在等,现在她真的在你怀里了。"
        )
    fridge = (
